<div class="container mx-auto px-4">
    ...
    @if ($this->id)
        ...
        @elsecan('vote')
            @include('vote', ['candidate' => $this])
        @endcan
    @else
        <div class="flex justify-between items-center">
        </div>
        <div class="flex row-auto gap-2">
            <label class="w-full block text-sm font-medium text-gray-900 dark:text-gray-400" for="search">
        ...
        @endif
        <div class="mt-4">
            <x-table :results="$results" :type="'candidate'" :create="true" :fillables="$fillables"
                     :warning="$warning"/>
        </div>
    @endif
</div>
